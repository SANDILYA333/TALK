/**
 * TALK Cryptographic Session Manager (Feature 2 — Phase 3)
 * 
 * Orchestrates session discovery, X3DH handshake execution, and session persistence
 ...
 * @returns {Promise<{ session: object, x3dhHeader?: object, isNewSession: boolean }>}
 */
export async function establishSessionWithPeer({
  ...
  // 4. Construct and persist session record
  const sessionRecord = {
...
/**
 * Handles an incoming initial message containing an X3DH header and derives matching session state.
 * 
 * @param {object} params
 ...
 * @returns {Promise<object>} Established session record
 */
export async function handleIncomingX3DHHandshake({
  ...
  // 3. Construct and persist receiver session record
  const sessionRecord = {
    sessionId: x3dhResult.sessionId,
